...
class ImportCityEventsCronViewTests(TestCase):
    @patch('agenda.views.call_command')
    def test_cron_endpoint_rejects_requests_without_secret_configured(self, mocked_call_command):
        response = self.client.post(reverse('agenda:import_city_events_cron'))
    ...
    @patch('agenda.views.call_command')
    def test_cron_endpoint_requires_post_method(self, mocked_call_command):
        response = self.client.get(reverse('agenda:import_city_events_cron'))

        self.assertEqual(response.status_code, 405)
        mocked_call_command.assert_not_called()

    @patch.dict('os.environ', {'CRON_SECRET': 'top-secret'})
    @patch('agenda.views.call_command')
    def test_cron_endpoint_rejects_invalid_authorization_header(self, mocked_call_command):
        response = self.client.post(
            reverse('agenda:import_city_events_cron'),
    ...
    @patch.dict('os.environ', {'CRON_SECRET': 'top-secret'})
    @patch('agenda.views.call_command')
    def test_cron_endpoint_runs_import_with_valid_authorization_header(self, mocked_call_command):
        response = self.client.post(
            reverse('agenda:import_city_events_cron'),
